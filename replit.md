...
- June 16, 2025. Redesigned "curious observer" achievement system - HOVER_AFTER_RESET dialog now triggers first when user hovers over contract/social links, then unlocks achievement. Requirements: Digital Odyssey + Dream Escapist achievements OR hasBeenThrown + userHasBeenReturn flags. After completion, all main dialogs, hover dialogs, contract dialogs, RETURN_DIALOG, and idle warnings (first, second, final) are disabled to provide a clean experience for users who have completed the achievement sequence.
