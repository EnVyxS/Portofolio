...
export const IDLE_DIALOGS = {
  ...
  FINAL_WARNING:
    "Kau pikir ini lucu?! Menatapku selama SEMBILAN MENIT PENUH seperti orang gila?! PERGI SANA, BRENGSEK!!",

  // Dialog setelah user menekan APPROACH HIM lagi
  RETURN_DIALOG: "Apa lagi maumu, sampah?! Berani-beraninya kembali setelah apa yang terjadi!",

  // Dialog setelah user melakukan hover
  HOVER_AFTER_RESET:
    "Hmph... Akhirnya kau bergerak juga... Terserah kau saja. Mau mengecek apa atau langsung tanda tangani kontrak sialan ini?",

  // Dialog untuk hover berlebihan
  EXCESSIVE_HOVER_WARNING: "TERUSLAH MEMAKSA, DAN KAU AKAN MENYESALINYA!",

  // Dialog terakhir sebelum 'diusir'
  FINAL_HOVER_WARNING: "AKU SUDAH MUAK DENGAN PERMAINANMU!",
};
...
class IdleTimeoutController {
  ...
  private showIdleWarning(text: string): void {
    ...
    const isAngryDialog = text.includes("TERUSLAH MEMAKSA") || 
                         text.includes("PERGI SANA") || 
                         text.includes("MUAK") ||
                         text.includes("MENYESALINYA");
    ...
    try {
      ...
      if (text.includes("SEMBILAN MENIT PENUH")) {
      ...
      else if (text.includes("Apa lagi maumu, sampah")) {
      ...
      else if (text.includes("Akhirnya kau bergerak juga")) {
      ...
      // Dialog untuk hover berlebihan
      else if (text.includes("TERUSLAH MEMAKSA")) {
  ...
  private throwUser(): void {
    ...
    const throwText = "Cukup sudah! MENYINGKIR DARI HADAPANKU!";
  ...
  private punchUser(): void {
    ...
    const punchText = "KAU MENANTANG ORANG YANG SALAH!";
    ...
    // Display the warning message only if audio/dialogs are completed
    if (!this.isAudioOrDialogActive()) {
      // Mostrar el mensaje de advertencia
      this.showIdleWarning(punchText);
      
      // Esperar a que termine el diálogo antes de ejecutar el efecto de golpe
      // Verificación regular para asegurarse de que el audio/diálogo ha terminado
      const checkAudioComplete = () => {
        if (!this.isAudioOrDialogActive()) {
          console.log("[IdleTimeoutController] Dialog/Audio completed, proceeding with punch effect");
          executePunch();
        } else {
          // Si el audio/diálogo sigue activo, verificar nuevamente en unos segundos
          setTimeout(checkAudioComplete, 500);
        }
      };
      
      // Iniciar verificación después de un breve retraso para permitir que el diálogo comience
      setTimeout(checkAudioComplete, 500);
    } else {
      // Si hay audio/diálogo activo, intentar mostrar el mensaje nuevamente después de un retraso
      setTimeout(() => {
        this.punchUser();
      }, 1000);
      return;
    ...
    // Set timer to redirect to dream/nightmare page
    // This redirection happens whether the audio/dialog completes or not
    setTimeout(() => {
      console.log("[IdleTimeoutController] Redirecting to dream page after blackout effect");
      
      try {
        const achievementController = AchievementController.getInstance();
        // Cek apakah sedang di halaman dream.html untuk mengetahui jika sudah di halaman nightmare
        const isDreamPage = window.location.href.includes("dream.html");
        
        if (!isDreamPage) {
          // Tandai achievement untuk dream/nightmare sequence
          // akan ditampilkan setelah redirect
          console.log("Nightmare achievement will only be shown on dream.html page");
        } else {
          // Jika sudah di halaman dream.html, langsung unlock achievement
          achievementController.unlockAchievement("nightmare");
        }

        // Temporary disable localStorage to make testing easier
        console.log("Achievement saving disabled for testing");
      } catch (error) {
        console.error("Error handling nightmare achievement:", error);
      }
      
      // Redirect to dream/nightmare page if not already there
      if (!window.location.href.includes("dream.html")) {
        window.location.href = "/dream.html";
      }
    }, 3000); // Add delay for dramatic effect, reduced from 4000ms to 3000ms
  }

  // Method untuk reset seluruh state
  public resetAll(): void {
    this.clearAllIdleTimers();
    this.clearAllHoverTimers();
    
    // Reset semua flag idle warning state
    this.hasShownFirstWarning = false;
    this.hasShownSecondWarning = false;
    this.hasShownFinalWarning = false;
    this.hasBeenThrown = false;
    
    // Reset semua flag hover berlebihan
    this.hasShownExcessiveHoverWarning = false;
    this.hasShownFinalHoverWarning = false;
    this.hasBeenPunched = false;
    
    // Reset flag has been reset
    // tapi jangan reset hasInteractedAfterReset (biarkan dihandle oleh interaksi pertama setelah reset)
    console.log("[IdleTimeoutController] Reset all idle state flags");
  }
}
